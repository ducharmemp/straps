...
return function(input, ctx)
  ...
  -- Warn (never refuse) when a new tool's source appears to call a blocking
  -- wait. straps.guard (config.wait_guard, default on) makes vim.system():wait()
  -- and vim.wait() RAISE when called inside a run, instead of silently
  -- blocking Neovim's main loop — this warning catches it at define time,
  -- before the tool is ever invoked. vim.fn.system is NOT guarded (it is a
  -- magic table, riskier to wrap, and short legitimate in-run uses are
  -- common), so for that one this warning is the only signal there is.
  if kind == "tool" and type(input.source) == "string" then
    ...
    if blocking then
      result = result .. "\nwarning: this tool source appears to call a blocking wait"
        .. " (vim.system():wait(), vim.wait, or vim.fn.system). vim.system():wait() and"
        .. " vim.wait() will ERROR if this tool ever runs inside an agent run"
        .. " (straps.guard); vim.fn.system will silently freeze the editor instead."
        .. " Use ctx.await, or ctx.system(cmd, opts) as shorthand, for subprocess/timer work."
    end
  end
  return result
end
