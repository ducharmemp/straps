-- tests/agents_buffer_spec.lua — the agents buffer (straps://agents): a single
...
it("zero sessions renders the empty-state line", function()
...
step(function()
  ...
  loaded_buf = state.new_session()
  -- running: a parked provider session.
  define_parked_provider()
  running_buf = state.new_session()
...
it("all_sessions classifies each session into exactly one set", function()
...
it("render shows sections in order, omits empty, map resolves", function()
  local buf = ui.open_agents("")
...
it("render is idempotent: two renders, identical lines and extmark count", function()
  local buf = ui.open_agents("")
...
it("<CR> on a saved row resumes it; on a loaded row shows it", function()
  local buf = ui.open_agents("")
...
it("<CR> replaces the agents buffer in the current window (oil-style), jumplist set", function()
...
it("cr on a stale loaded row notifies and re-renders", function()
...
it("x on the running row stops it", function()
  local buf = ui.open_agents("")
  local ri = line_index(buf, "━━ running")
  local li = line_index(buf, "━━ loaded")
  local run_lnum
...
it("x and i on a non-running row notify without error", function()
  local buf = ui.open_agents("")
...
it("refresh debounces back-to-back calls into one render", function()
  local buf = ui.open_agents("")
...
it("rows are capped to one legible line", function()
...
it("loaded row shows the transcript age", function()
  local buf = ui.open_agents("")
  local ls = lines_of(buf)
  local row
  for lnum = 1, #ls do
...
it("loaded row shows context fill when usage is present", function()
...
it("agents_winbar = false puts the legend on the first line, no winbar", function()
...
it("ui.winbar dispatches per buffer: session, agents, other", function()
...
it("show_session clears an inherited dispatcher when session_winbar = false", function()
...
it("a throwing fn.agents_winbar degrades to an empty winbar", function()
...
it("agents window gets list options and cursor on a row", function()
  local buf = ui.open_agents("")
