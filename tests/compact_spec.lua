-- tests/compact_spec.lua — fn.compact, state.list_blocks and auto-compaction.
--   busted tests/compact_spec.lua
...
it("state.list_blocks indexes the transcript", function()
...
local summary
it("fn.compact shrinks old tool blocks and reports it", function()
...
it("last 2 assistant turns untouched, older ones stubbed", function()
...
it("parse stays valid: alternating roles, ids intact, old inputs {}", function()
...
it("second fn.compact is a stable no-op", function()
...
it("auto_compact_bytes triggers fn.compact mid-run and logs it", function()
...
it("auto_compact_tokens triggers and logs est_tokens", function()
  local logfile = vim.fn.tempname()
...
it("growth guard: compaction fires once, not every turn, near the limit", function()
...
it(":StrapsCompact command smoke", function()
