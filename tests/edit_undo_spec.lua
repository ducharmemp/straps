-- tests/edit_undo_spec.lua — native-undo edits (tool.write_file, tool.edit_file).
--   busted tests/edit_undo_spec.lua
...
it("write_file creates a new file: disk matches, buffer loaded, changedtick rose", function()
...
it("write_file then edit_file: a single in-buffer undo reverts the edit; redo restores", function()
  local path = vim.fn.tempname() .. ".txt"
...
it("edit_file 0-match errors and leaves the file unchanged", function()
  local path = vim.fn.tempname() .. ".txt"
...
it("edit_file >1-match without replace_all errors and leaves the file unchanged", function()
  local path = vim.fn.tempname() .. ".txt"
...
it("edit_file replace_all changes every occurrence in one undoable step", function()
  local path = vim.fn.tempname() .. ".txt"
...
it("read_file/edit_file share the live buffer source of truth", function()
  local path = vim.fn.tempname() .. ".txt"
...
it("patch_file applies multiple line hunks through the live buffer as one undo step", function()
  local path = vim.fn.tempname() .. ".txt"
  write_file({ path = path, content = "one\ntwo\nthree\nfour\n" })
  local buf = bufnr_for(path)
  local before = buf_text(buf)
...
it("patch_file refuses overlapping hunks before changing the file", function()
  local path = vim.fn.tempname() .. ".txt"
...
it("patch_file refuses stale expected_old_text before changing the file", function()
  local path = vim.fn.tempname() .. ".txt"
  write_file({ path = path, content = "a\nb\nc\n" })
  local buf = bufnr_for(path)
  local ok, err = pcall(patch_file, { path = path, hunks = {
...
it("hook.after_write still fires and its return is appended to the result", function()
  registry.define({
