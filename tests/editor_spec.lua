-- tests/editor_spec.lua — editor-native tools (straps.editor).
--   busted tests/editor_spec.lua
...
it("editor read-only tools registered as kind 'tool' with an input_schema", function()
...
it("diagnostics formats seeded diagnostics as file:line:col: SEVERITY message", function()
...
it("diagnostics with no diagnostics returns 'no diagnostics'", function()
  local path = vim.fn.tempname() .. ".txt"
...
it("diagnostic_at and diagnostic_next expose position-oriented diagnostics", function()
  local path = vim.fn.tempname() .. ".txt"
  write_file(path, "one\ntwo\nthree\n")
  local buf = vim.fn.bufadd(path)
  vim.fn.bufload(buf)
...
it("fix_diagnostic reports no diagnostic before asking LSP", function()
  local path = vim.fn.tempname() .. ".txt"
  write_file(path, "clean\n")
...
if not has_lua_parser then
  ...
  it("symbols lists both functions with line ranges (tree-sitter)", function()
  ...
  it("tree_sitter_status and node_at expose parser/node details", function()
  ...
  it("read_node returns an enclosing tree-sitter node with numbered lines", function()
  ...
  it("read_symbol returns the body of a named function (numbered)", function()
  ...
  it("read_symbol on an unknown name returns the not-found message", function()
...
it("definition with no LSP client returns the graceful fallback", function()
...
it("references with no LSP client returns the graceful fallback", function()
  local path = vim.fn.tempname() .. ".py"
  write_file(path, "y = 2\n")
  local out = drive(function(ctx)
...
it("declaration/type_definition/implementation no-client messages name the method", function()
  local path = vim.fn.tempname() .. ".py"
  write_file(path, "z = 3\n")
...
it("lsp_status reports no attached client for a file", function()
  local path = vim.fn.tempname() .. ".py"
  write_file(path, "z = 3\n")
  local out = drive(function(ctx)
...
it("expanded editor read-only tools are auto-allowed by hook.confirm", function()
...
it("move_file: plain move (no LSP client) relocates the file, creating dirs", function()
...
it("move_file: missing source errors with a clear message", function()
  local out = drive(function(ctx)
...
it("move_file: existing destination refuses rather than overwriting", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
...
it("move_file requires both from and to", function()
...
it("move_file is registered with the right schema and requires confirmation", function()
  local e = registry.get("tool.move_file")
  assert(e, "tool.move_file not registered")
  assert(e.fn == nil, "tool.move_file view must not expose fn")
...
it("move_files: plain batch (no LSP client) moves every file", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
...
it("move_files: one invalid entry refuses the WHOLE batch (no partial move)", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
  local c = dir .. "/c.txt"
  write_file(c, "C\n")
...
it("move_files: two entries targeting the same destination are refused", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
...
it("move_files: from == to in one entry is refused", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
...
it("move_files requires a non-empty moves array", function()
...
it("move_files is registered with the right schema and requires confirmation", function()
  local e = registry.get("tool.move_files")
  assert(e, "tool.move_files not registered")
  assert(e.fn == nil, "tool.move_files view must not expose fn")
  local req = e.input_schema and e.input_schema.required
...
it("delete_file: plain delete (no LSP client) removes the file", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
  local p = dir .. "/a.txt"
  write_file(p, "A\n")
...
it("delete_file: missing file errors with a clear message", function()
  local out = drive(function(ctx)
...
it("delete_file: refuses when the buffer has unsaved changes", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
  local p = dir .. "/b.txt"
  write_file(p, "B\n")
...
it("delete_file requires path", function()
...
it("delete_file is registered with the right schema and requires confirmation", function()
  local e = registry.get("tool.delete_file")
  assert(e, "tool.delete_file not registered")
  assert(e.fn == nil, "tool.delete_file view must not expose fn")
  local req = e.input_schema and e.input_schema.required
...
it("delete_files: plain batch (no LSP client) deletes every file", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
  local a, b = dir .. "/a.txt", dir .. "/b.txt"
  write_file(a, "A\n")
...
it("delete_files: one invalid entry refuses the WHOLE batch (no partial delete)", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
  local c = dir .. "/c.txt"
  write_file(c, "C\n")
...
it("delete_files: a duplicate path in the batch is refused", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
  local p = dir .. "/d.txt"
  write_file(p, "D\n")
...
it("delete_files: an unsaved buffer among many entries refuses the whole batch", function()
  local dir = vim.fn.tempname()
  vim.fn.mkdir(dir, "p")
  local a, b = dir .. "/a.txt", dir .. "/b.txt"
  write_file(a, "A\n")
...
it("delete_files requires a non-empty paths array", function()
...
it("delete_files is registered with the right schema and requires confirmation", function()
  local e = registry.get("tool.delete_files")
  assert(e, "tool.delete_files not registered")
  assert(e.fn == nil, "tool.delete_files view must not expose fn")
  local req = e.input_schema and e.input_schema.required
...
it("core editor tools are auto-allowed by hook.confirm", function()
