-- tests/excise_spec.lua — tool.transcript_excise: agent-directed context surgery.
--   busted tests/excise_spec.lua
...
it("list mode indexes blocks, sizes and locks", function()
...
it("list mode is read-only per fn.readonly_policy; excise mode is not", function()
...
it("list mode does not modify the transcript", function()
  local bufnr = build_transcript()
...
it("excising reclaims bytes and leaves a receipt", function()
  local bufnr = build_transcript()
...
it("parse stays valid after excision: pairing, alternation, first role", function()
  local bufnr = build_transcript()
...
it("excised prose blocks keep non-empty content (never dropped by parse)", function()
  local bufnr = build_transcript()
...
it("inline text on a prose marker line is reclaimed too", function()
  local bufnr = build_transcript()
...
it("the system block is never excisable", function()
  local bufnr = build_transcript()
...
it("the turn in flight is never excisable", function()
  local bufnr = build_transcript()
...
it("a note is required to excise", function()
  local bufnr = build_transcript()
...
it("a marker-shaped note cannot inject a block", function()
  local bufnr = build_transcript()
...
it("excise is idempotent", function()
  local bufnr = build_transcript()
...
it("out-of-range and non-existent blocks are refused, not crashed", function()
  local bufnr = build_transcript()
  local before = buf_text(bufnr)
...
it("hostile input never crashes and never corrupts the transcript", function()
...
it("an unbounded range is clamped, not iterated", function()
...
it("a block smaller than its own receipt is left alone", function()
  local bufnr = build_transcript()
...
it("a note is flattened, stripped of control bytes and capped", function()
  local bufnr = build_transcript()
  excise(bufnr, {
    blocks = { idx(bufnr, "tool_result", 1) },
...
it("works on a transcript that is not file-backed", function()
...
it("multi-block excision is bottom-up: no block is eaten", function()
  local bufnr = build_transcript()
  local n_before = #state.list_blocks(bufnr)
  local kinds_before = {}
...
it("one call is one undoable step", function()
  local bufnr = build_transcript()
...
it("only a real child session can be targeted", function()
  local bufnr = build_transcript()
...
it("a parent can excise its child's transcript, marked as such", function()
...
it("an agent excises its own context mid-run and the run completes", function()
  _G.excise_calls = 0
...
it("the excised transcript survives a reload from disk", function()
  local bufnr = build_transcript()
