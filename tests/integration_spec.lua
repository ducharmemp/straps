...
-- Run: busted tests/integration_spec.lua
...
it("setup registers provider, tools, hooks", function()
...
it("run completed", function()
  assert(not loop.running(bufnr), "loop still running")
  assert(_G.__it.turn == 3, "expected 3 provider turns, got " .. _G.__it.turn)
end)

it("hook.after_write redefinition is SESSION-scoped: chain sees v2, global stays v1", function()
...
it("new tool is in the tool list on the following turn", function()
...
it("write_file wrote the file and the linter hook fired into its result", function()
...
it("agent-defined tool executed", function()
  assert(text:find("IT WORKS", 1, true), "shout result missing")
end)

it("final assistant text streamed into the buffer", function()
  assert(text:find("done: linting is now automatic", 1, true))
end)

it("transcript still parses; messages stay API-valid", function()
...
do
  ...
  local registry
  step(function()
    registry = require("straps.registry")
  end)
  ...
  it("pick_model sets config.model from a picked entry", function()
  ...
  it("pick_model writes config.openai_model and uses only OpenAI models when OpenAI is active", function()
  ...
  it("pick_model cancel (nil choice) leaves config.model untouched", function()
    straps.config.model = "claude-sonnet-5"
    with_list_models("return function() return nil, 'stubbed offline' end", function()
  ...
  it("pick_model merges live-discovered models into the picker list", function()
...
return function()
  ...
  it("pick_effort sets config.effort from a picked entry", function()
  ...
  it("pick_model on a session buffer sets the provider-specific PER-BUFFER slot", function()
  ...
  it("pick_effort on a session buffer sets it PER-BUFFER, not global", function()
    local sess = state.new_session()
    local prev = vim.api.nvim_get_current_buf()
    vim.api.nvim_set_current_buf(sess)
  ...
  do
    ...
    step(function()
      ...
      pdir = vim.fn.tempname()
      vim.fn.mkdir(pdir, "p")
      vim.env.XDG_CONFIG_HOME = pdir
    end)

    it("pick_provider global sets config.provider AND persists to the file", function()
    ...
    it("pick_provider cancel (nil) leaves config.provider untouched", function()
      straps.config.provider = "anthropic"
      with_stub(nil, function()
        ui.pick_provider()
      end)
    ...
    it("pick_provider on a session buffer sets it PER-BUFFER, no file write", function()
      local sess = state.new_session()
      local prev = vim.api.nvim_get_current_buf()
      vim.api.nvim_set_current_buf(sess)
      straps.config.provider = nil
...
it("session_status reflects per-buffer override with a divergence marker", function()
...
it("session_status and session_winbar are empty on non-session buffers", function()
  local ui = require("straps.ui")
...
it("usage_status: empty with no usage, and off a session buffer", function()
  local ui = require("straps.ui")
  local sess = state.new_session()
  local prev = vim.api.nvim_get_current_buf()
  vim.api.nvim_set_current_buf(sess)
...
it("usage_status: context fill percent and cache rate from vim.b.straps_usage", function()
  local ui = require("straps.ui")
  local sess = state.new_session()
  local prev = vim.api.nvim_get_current_buf()
  vim.api.nvim_set_current_buf(sess)
...
it("usage_status: unknown model with no context_window shows raw count", function()
  local ui = require("straps.ui")
  local sess = state.new_session()
  local prev = vim.api.nvim_get_current_buf()
  vim.api.nvim_set_current_buf(sess)
...
it("session_winbar includes the usage segment when usage is present", function()
  local ui = require("straps.ui")
  local sess = state.new_session()
  local prev = vim.api.nvim_get_current_buf()
  vim.api.nvim_set_current_buf(sess)
...
it("session_info returns nil off a session buffer and for an invalid bufnr", function()
  local ui = require("straps.ui")
  local scratch = vim.api.nvim_create_buf(false, true)
...
it("session_info resolves model/effort/status for a NON-current buffer", function()
  local ui = require("straps.ui")
  local sess = state.new_session()
  straps.config.model = "claude-sonnet-5"
  straps.config.effort = "off"
...
it("session_info.overridden means DIVERGENCE, not merely a var being set", function()
  local ui = require("straps.ui")
  local sess = state.new_session()
  straps.config.model = "claude-sonnet-5"
  straps.config.effort = "off"
...
it("session_info percentages are pre-rounded integers", function()
  local ui = require("straps.ui")
  local sess = state.new_session()
  straps.config.model = "claude-sonnet-5"      -- context = 200000
...
it("usage_status gates on the session buffer, not merely on straps_usage", function()
  local ui = require("straps.ui")
...
it("human_tokens formats the counts components share", function()
  local ui = require("straps.ui")
...
it("default progress hook cleaned up its extmarks", function()
...
it("typing under the trailing user marker and <CR> starts a run", function()
...
it("<CR> while a run is active prompts to steer instead of starting a second run", function()
