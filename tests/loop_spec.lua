-- tests/loop_spec.lua — agent loop tests. Run from the repo root:
--   busted tests/loop_spec.lua
...
it("provider entries registered and compiled", function()
...
it("tool loop with mid-run tool.ping redefinition", function()
...
it("cancel mid-batch stubs results for unexecuted tool_use blocks", function()
  allow_all()
...
it("confirm denial yields is_error tool_result and the run continues", function()
...
it("provider error is appended as an assistant block and the run ends", function()
...
it("loop.stop mid-await cancels cleanly", function()
...
it("VimLeavePre fires active runs' cancel fns (kills orphaned processes)", function()
  allow_all()
...
it("empty conversation fails fast with a helpful message, no provider call", function()
...
it("trailing assistant message fails fast with a helpful message, no provider call", function()
  define("fn.provider", "fn", "test: must not be called", [==[
...
it("stop_reason tool_use with no tool blocks errors, not a resend or user-blaming message", function()
  _G.straps_test_calls = 0
...
it("steering message is delivered on the next turn", function()
  allow_all()
  _G.straps_test_calls = 0
  _G.straps_test_reqs = {}
...
it("progress events fire in order with a final done", function()
  allow_all()
  define("tool.ping", "tool", "ping", [[return function() return "pong" end]])
...
it("max_turns exhaustion is loud", function()
  allow_all()
  define("tool.ping", "tool", "ping", [[return function() return "pong" end]])
...
it("stall detector stops an all-errors loop before max_turns", function()
  allow_all()
...
it("stall detector catches an exact-repeat loop", function()
  allow_all()
  define("tool.ping", "tool", "ping", [[return function() return "pong" end]])
  local straps = require("straps")
  local saved_max, saved_stall = straps.config.max_turns, straps.config.stall_limit
...
it("a repeat mixed with a new call is not a stall", function()
  allow_all()
  define("tool.ping", "tool", "ping", [[return function() return "pong" end]])
  local straps = require("straps")
  local saved_max, saved_stall = straps.config.max_turns, straps.config.stall_limit
...
it("a productive turn resets the stall counter", function()
  allow_all()
  define("tool.ping", "tool", "ping", [[return function() return "pong" end]])
...
it("fn.log writes structured events", function()
  allow_all()
  define("tool.ping", "tool", "ping", [[return function() return "pong" end]])
  local straps = require("straps")
...
it("thinking event carries max", function()
  allow_all()
  local straps = require("straps")
  local saved_max = straps.config.max_turns
...
it("provider usage is accumulated onto vim.b.straps_usage", function()
  allow_all()
...
it("a blank turn after a tool call is nudged, then ends loudly if still blank", function()
  allow_all()
  define("tool.ping", "tool", "ping", [[return function() return "pong" end]])
  local straps = require("straps")
...
it("a blank turn on max_tokens ends loudly with no nudge", function()
  allow_all()
  local straps = require("straps")
  _G.straps_test_progress = {}
  define("hook.on_progress", "hook", "test: record events", [[
...
it("a normal end_turn WITH text is still a clean ok (no false blank)", function()
  allow_all()
  _G.straps_test_progress = {}
  define("hook.on_progress", "hook", "test: record events", [[
return function(ev) _G.straps_test_progress[#_G.straps_test_progress + 1] = ev end
...
it("oversized tool result truncates on a UTF-8 char boundary", function()
  allow_all()
  local straps = require("straps")
...
it("a stopped run continues from where it left off", function()
  allow_all()
...
it("an interrupted transcript (tool_use, no result) is healed and continues", function()
  allow_all()
...
it("heal pairs only the unfinished calls of an interrupted batch", function()
...
it("heal refuses to touch a session with a live run", function()
...
it("heal leaves a hand-mangled transcript alone", function()
...
it("heal is a no-op on a well-formed transcript", function()
  local bufnr = state.new_session()
