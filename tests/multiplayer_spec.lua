-- tests/multiplayer_spec.lua — multiplayer awareness: an agent noticing that
-- other agents share this Neovim.
--   busted tests/multiplayer_spec.lua
...
it("peer_agents excludes self and reports running vs idle", function()
...
it("peer_agents classifies parent, child and sibling relations", function()
...
it("peer_agents attributes written files to the session that wrote them", function()
  local me = state.new_session()
  local other = state.new_session()
...
it("peer_agents sorts running sessions before idle ones", function()
  local me = state.new_session()
...
it("peer_agents tolerates an invalid calling bufnr and dead buffers", function()
...
it("tool.agents is registered with an empty-object schema", function()
...
it("tool.agents reports solitude when no other session exists", function()
...
it("tool.agents lists peers with relation, state, task and files", function()
  local me = state.new_session()
  local child = state.new_session()
  vim.b[child].straps_parent = me
...
it("tool.agents is auto-allowed and parallel-safe", function()
...
it("on_run_start announces a running peer once, into the transcript", function()
...
it("on_run_start re-announces when the peer set changes", function()
  local me = session_with_prompt()
...
it("on_run_start is silent when no other agent is running", function()
  local me = session_with_prompt()
...
it("on_run_start never makes an unsendable transcript sendable", function()
...
it("on_run_start tolerates an invalid session buffer", function()
  local dead = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_delete(dead, { force = true })
...
it("skill.multiplayer ships, is listed, and loads", function()
