-- tests/private_surface_spec.lua — the registry's private surface.
--   busted tests/private_surface_spec.lua
...
it("registry.entries and registry.scopes are not exposed", function()
  assert(registry.entries == nil, "registry.entries must be private")
  assert(registry.scopes == nil, "registry.scopes must be private")
end)

it("get() returns an inert copy: no fn, and writes do not reach the live entry", function()
...
it("hook_entries() returns copies without fn", function()
...
it("bind() returns the compiled fn for hook/fn and refuses tool.*", function()
...
it("bind() supports the documented chaining idiom without recursion", function()
...
it("scope_parent reports existence and parent; parents are fixed at creation", function()
...
it("grants live in the registry, not in vim.b", function()
...
it("writing vim.b straps_allowed no longer auto-allows (counterfactual for the old hole)", function()
  local buf = state.new_session()
...
it("direct registry.call(tool.*) passes the before-guards and re-raises the original error", function()
...
it("a tool body cannot grant its own session", function()
...
it("a tool body abandoned mid-await does not refuse the session's grants forever", function()
  define("hook.confirm", "hook", "test: allow everything", "return function() return true end")
...
it("spawn-style grants to ANOTHER session from a tool body are permitted", function()
...
it("a tool body that resets the active scope does not leak past its block", function()
