...
-- Run: busted tests/project_spec.lua
...
-- ---------------------------------------------------------------- no file
it("no .straps.lua anywhere upward: false, 'none', no error", function()
...
it("untrusted file + headless confirm (0): not executed, no error", function()
  cd(proj)
  local ok, loaded, info = pcall(straps.load_project_registry, { store_path = store })
  cd(orig_cwd)
  assert(ok, "load_project_registry threw: " .. tostring(loaded))
...
-- ---------------------------------------------------------------- trust_all
it("opts.trust_all executes and records path + hash in the store", function()
  cd(proj)
...
-- ------------------------------------------------------- once-per-content
it("second call without trust_all: trusted via store, once-guard skips re-exec", function()
  cd(proj)
...
-- ------------------------------------------------------------ changed file
it("changed content is untrusted again: not executed", function()
  write_file(proj .. "/.straps.lua", MARKER_SRC .. "\n-- edited after trust\n")
  cd(proj)
  local ok, loaded, info = pcall(straps.load_project_registry, { store_path = store })
  cd(orig_cwd)
...
it("trust_all does not record a hash when .straps.lua fails to execute", function()
...
-- ------------------------------------------------------------ seq ordering
it("project-defined tool lands AFTER builtins in names_by_seq order", function()
...
-- --------------------------------------------------------------- ui path
it("ui.open_session() loads a trusted .straps.lua from cwd", function()
...
it("missing global file: false, 'none', no error, no execution", function()
...
local gpath
step(function()
  gpath = tmp .. "/globalcfg/straps/init.lua"
  vim.fn.mkdir(vim.fn.fnamemodify(gpath, ":h"), "p")
  write_file(gpath, GLOBAL_SRC)
end)

it("present global file loads without any trust prompt", function()
...
it("global corpus does not depend on cwd (loads from a bare dir)", function()
...
it("once-per-session guard: same path does not re-execute", function()
  local loaded, info = straps.load_global_registry({ path = gpath })
...
it("broken global file: no throw, not marked loaded (retried next session)", function()
