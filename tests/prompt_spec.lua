...
-- ------------------------------------------------------------- composition
it("fn.system_prompt composes core + env; project stays out of a bare dir", function()
  cd(deep)
...
-- ------------------------------------------------------------ project files
it("project layer includes AGENTS.md, CLAUDE.md and instructions_files", function()
...
-- ---------------------------------------------------------------- layering
it("upward AGENTS.md files layer, ancestor first and nearest last", function()
...
-- ------------------------------------------------------------- late binding
it("composition is late-bound: redefining a layer changes fn.system_prompt", function()
...
-- --------------------------------------------------------------- new_session
it("new_session writes the composed prompt into the system block", function()
...
-- -------------------------------------------------------- untrusted content
it("core layer draws the untrusted-content boundary", function()
...
it("the [straps] harness-speech convention is declared, and to subagents too", function()
...
-- ---------------------------------------------------------- subagent variant
it("core layer adapts for subagents: no # Subagents, child section appended", function()
  local core = registry.call("fn.system_prompt_core")
...
it("subagent opts add readonly and tool-restriction notes", function()
...
it("fn.system_prompt forwards opts to the core layer only", function()
...
it("new_session forwards opts so a spawned child's system block adapts", function()
...
-- ----------------------------------------------------------- self-extension
it("core layer carries the self-extension triggers and .straps.lua persistence", function()
  local core = registry.call("fn.system_prompt_core")
...
-- fixture.
it("env layer reports version control from a git repo root", function()
...
-- ------------------------------------------------------------ env top-level
it("env layer lists the top-level shape of cwd, dirs first", function()
  cd(root)
  local ok, err = pcall(function()
    local env = registry.call("fn.system_prompt_env")
...
-- ----------------------------------------------------- layer contribution
it("a new fn.system_prompt_layer.* entry appends a section at the end", function()
  registry.define({
...
it("an empty fn.system_prompt_layer.* result drops the section", function()
  registry.define({
    name = "fn.system_prompt_layer.zzz",
    kind = "fn",
...
it("relative order of the four builtin layers is preserved", function()
  local prompt = registry.call("fn.system_prompt")
