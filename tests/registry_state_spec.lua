...
-- Run from the repo root: busted tests/registry_state_spec.lua
...
it("redefine is late-bound at existing call sites", function()
...
it("bad source rejected, old entry kept", function()
...
it("define validates name and kind", function()
...
it("call errors on missing, try_call returns nil", function()
...
it("names is sorted and filters by kind", function()
...
it("define_default only defines when absent", function()
...
it("scope-aware remove un-shadows a session-scoped entry", function()
...
it("hook.on_define fires and cannot break define", function()
  local seen = {}
...
it("render -> execute round-trip", function()
  registry.define{
...
it("render bumps bracket level on collision", function()
...
it("dump restores the registry", function()
...
it("new_session: system block + empty trailing user", function()
...
it("transcript parse: roles merged, ids intact", function()
  local bufnr = state.new_session()
...
it("adjacent same-role messages merged", function()
  local bufnr = state.new_session()
...
it("empty assistant block omitted", function()
  local bufnr = state.new_session()
...
it("tool blocks with undecodable attrs are skipped, not emitted null", function()
...
it("orphaned tool_result downgrades to user text", function()
...
it("orphan downgrade never precedes a valid tool_result", function()
...
it("duplicate tool_result for one id downgrades", function()
  local bufnr = state.new_session()
  state.append(bufnr, "user", nil, "go")
  state.append(bufnr, "tool_use", { id = "t1", name = "ping" }, "{}")
...
it("tool_result stranded past a later assistant message downgrades", function()
  local bufnr = state.new_session()
  state.append(bufnr, "user", nil, "go")
  state.append(bufnr, "tool_use", { id = "t1", name = "ping" }, "{}")
...
it("empty assistant marker between use and result does not orphan", function()
...
it("escaping round-trips marker-like content", function()
  local bufnr = state.new_session()
...
it("append_text streams into the current block", function()
  local bufnr = state.new_session()
  state.append(bufnr, "user", nil, "go")
  state.append(bufnr, "assistant", nil, "")
...
it("ensure_trailing_user and last_user_text", function()
  local bufnr = state.new_session()
...
it("parse is total: garbage before first marker ignored", function()
...
it("new_session falls back without fn.system_prompt, uses it when present", function()
...
it("text typed on the user marker line is content, not dropped", function()
  local bufnr = vim.api.nvim_create_buf(true, false)
  vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, {
    "%%[straps:system]%%", "sys", "",
...
it("inline marker text concatenates with following lines", function()
  local bufnr = vim.api.nvim_create_buf(true, false)
  vim.api.nvim_buf_set_lines(bufnr, 0, -1, false,
...
it("JSON-looking inline text on a user marker stays content", function()
  local bufnr = vim.api.nvim_create_buf(true, false)
  vim.api.nvim_buf_set_lines(bufnr, 0, -1, false,
    { '%%[straps:user]%% {"id":"x"}' })
...
it("tool markers still parse JSON attrs, not inline content", function()
  local bufnr = vim.api.nvim_create_buf(true, false)
  vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, {
...
it("append_text glues onto a marker line that has inline content", function()
  local bufnr = vim.api.nvim_create_buf(true, false)
...
it("inline text on the trailing user marker counts for last_user_text", function()
  local bufnr = vim.api.nvim_create_buf(true, false)
  vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "%%[straps:user]%% hey" })
  eq(state.last_user_text(bufnr), "hey")
end)

it("seq is stamped at first define and survives redefinition", function()
...
it("dump restores entries in registration order", function()
...
it("setup{} with both layers left at their default registers editor + openai", function()
