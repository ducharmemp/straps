-- tests/render_spec.lua — the transcript rendering layer (fn.render + themed
...
-- ------------------------------------------------------------ highlight groups
it("highlight groups resolve to their default link targets", function()
  local want = {
...
it("a user `hi link` override survives re-apply (default = true)", function()
...
local buf
local before_text
local before_modified
local before_parse
step(function()
  buf = make_buf()
  before_text = vim.api.nvim_buf_get_lines(buf, 0, -1, false)
  before_modified = vim.bo[buf].modified
  before_parse = vim.inspect(state.parse(buf))
  registry.call("fn.render", buf)
end)

it("marker lines carry a conceal extmark and a role-rule overlay", function()
...
it("role rules read apart: per-role bar char + role-colored lead", function()
...
it("message body lines carry no render extmark", function()
  assert(#marks_on_row(buf, LN.user_body) == 0,
    "user body line has a render extmark (color must live only on marks)")
end)

it("tool marker lines are concealed with a command-style card header", function()
...
it("fn.render is idempotent", function()
...
it("render leaves buffer text, modified and parse unchanged", function()
...
-- --------------------------------------------------------------- fold summary
it("fold summary is a command-style colored chunk list — ok result", function()
...
it("fold summary reads is_error from the tool_result — error case", function()
...
-- --------------------------------------------------------- fn.tool_display
it("fn.tool_display renders each builtin as a command-style line", function()
...
it("fn.tool_display is defensive: bad/missing input never errors", function()
  local td = function(name, input) return registry.call("fn.tool_display", name, input) end
...
it("a broken fn.tool_display redefinition can't break fn.render", function()
...
it("redefining fn.tool_display changes the next fold summary (late-bound)", function()
  local reg = require("straps.registry")
  local original = reg.get("fn.tool_display").source
...
-- ------------------------------------------------------------- config.render
it("config.render = false places no extmarks", function()
...
-- --------------------------------------------------------------- foldexpr/text
it("foldexpr merges a tool_use/tool_result pair into one fold", function()
...
it("outline headline prefers text typed on the marker line itself", function()
...
it("system block folds to a labelled summary (not shown inline in full)", function()
  local buf = vim.api.nvim_create_buf(false, true)
...
it("the system block starts folded closed", function()
...
it("a transcript with no system block folds sanely too", function()
...
-- ------------------------------------------------------------- turn navigation
it("]] / [[ step between conversation turns", function()
  local win = vim.api.nvim_open_win(buf, true, {
...
it("gO outlines the transcript into the findings list", function()
...
it("ui.open_session()'s window actually gets fold options and navigation maps", function()
...
it("session window options are per-(window,buffer), never per-window", function()
...
it("agents window options are per-(window,buffer), never per-window", function()
