-- tests/reorg_spec.lua — the layer-split mechanical invariant.
--   busted tests/reorg_spec.lua
...
it("every reference name is present, in the same relative order", function()
  local pos = positions()
...
it("every layer skeleton sorts strictly after every reference name", function()
  local pos = positions()
...
it("every layer skeleton is an fn that returns the empty string", function()
  for _, s in ipairs(LAYER_SKELETONS) do
...
it("empty prompt fragments contribute nothing to the composition", function()
