-- tests/run_eval_lua.lua — output formatting of tool.eval_lua.
--   nvim --headless -l tests/run_eval_lua.lua
...
case("bare string prints verbatim (no quotes/escaping)", function()
...
case("flat list: one element per line, numbered", function()
  local out = ev([[return { "alpha", "beta", "gamma" }]])
  assert(out == "  1 | alpha\n  2 | beta\n  3 | gamma",
    "unexpected flat-list format:\n" .. out)
end)

case("flat list numbers are right-aligned to the widest index", function()
...
case("flat list of mixed scalars (number/boolean/string)", function()
  local out = ev([[return { 10, true, "s", false }]])
  assert(out == "  1 | 10\n  2 | true\n  3 | s\n  4 | false",
    "unexpected mixed-scalar format:\n" .. out)
end)

case("empty list renders as an explicit marker", function()
  assert(ev([[return {}]]) == "{} (empty list)", "empty list not marked")
end)

case("map/associative table falls back to vim.inspect", function()
...
case("list containing a table is NOT treated as flat (uses vim.inspect)", function()
...
case("single number returns via vim.inspect", function()
  assert(ev([[return 42]]) == "42", "number not formatted as 42")
end)

case("multiple return values are labeled and ordered", function()
...
case("single return value is NOT labeled", function()
  local out = ev([[return "solo"]])
  assert(out == "solo", "single value should be unadorned, got:\n" .. out)
end)

case("no return value yields nil", function()
  assert(ev([[local x = 1]]) == "nil", "no-return should be 'nil'")
end)

case("runtime error is reported, not raised", function()
...
case("compile error is reported as a load error", function()
  local out = ev([[this is not lua!!!]])
  assert(out:find("load error: ", 1, true) == 1, "compile error not reported as load error:\n" .. out)
end)

case("explicit nil return is preserved (n counts it)", function()
