-- tests/run_hook_fanout.lua — multi-subscriber hook fan-out (Contract 1).
--   nvim --headless -l tests/run_hook_fanout.lua
...
case("counterfactual: removing the extra subscriber drops its output", function()
...
case("counterfactual: a fixed subscriber no longer produces an error line", function()
...
case("hook_entries: exact name plus dotted suffixes, sorted by seq, kind filtered", function()
...
case("counterfactual: hook_entries with a name that has no subscribers returns empty", function()
  local entries = registry.hook_entries("hook.no_such_thing_at_all")
  assert(#entries == 0, "expected zero entries, got " .. #entries)
end)

case("call_hooks collects non-nil results in seq order", function()
...
case("call_hooks: one broken subscriber does not block the others; error names the entry", function()
...
case("counterfactual: fixing the broken subscriber empties the errors table", function()
...
case("hook.after_tool fold (via the real loop): each subscriber sees the previous one's result", function()
...
case("counterfactual: without the second subscriber the fold stops after the first", function()
  define("hook.confirm", "hook", "test: allow everything", "return function() return true end")
  define("tool.ping", "tool", "ping", [[return function() return "pong" end]])
