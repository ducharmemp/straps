-- tests/run_layers.lua — config-gated layer registration: both layers off.
--   nvim --headless -l tests/run_layers.lua
...
case("editor = false: editor tools absent", function()
...
case("editor = false: core tools stay registered", function()
...
case("editor = false: fn.build_tools omits editor tool names", function()
...
case("openai = false: openai-only fns absent, anthropic stays", function()
...
case("openai = false: fn.provider raises a clean disabled message", function()
...
case("openai = false: fn.list_models returns (nil, err), does not raise", function()
