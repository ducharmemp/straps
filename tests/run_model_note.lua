-- tests/run_model_note.lua — model awareness via the per-request system text:
...
-- user's voice).
--   nvim --headless -l tests/run_model_note.lua
...
case("fn.model_note names the effective model, provider and effort", function()
...
case("a configured label renders as 'label (id)'", function()
  local me = state.new_session()
...
case("effort defaults to 'off' when unset", function()
  local me = state.new_session()
  local note = registry.call("fn.model_note", { bufnr = me })
...
case("nil for a non-session buffer, bad ctx, and a dead buffer", function()
...
case("the loop appends the note to every request's system, tracking a mid-run switch", function()
...
case("a broken fn.model_note degrades to a note-less request, not a failed run", function()
...
case("the note points at the models tool", function()
  local me = state.new_session()
  local note = registry.call("fn.model_note", { bufnr = me })
