-- tests/run_progress.lua — the default progress display + stream-liveness
...
--   nvim --headless -l tests/run_progress.lua
...
case("liveness segment: not-streaming is empty, receiving vs silent by age", function()
...
case("thinking phase shows 'receiving' right after activity", function()
...
case("a running tool shows no liveness segment (local, no stream)", function()
  local buf = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_lines(buf, 0, -1, false, { "line" })
...
case("note_activity flips a stale 'silent' back to 'receiving'", function()
  local buf = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_lines(buf, 0, -1, false, { "line" })
  ui.progress(buf, { type = "thinking", turn = 1, max = 8 })
