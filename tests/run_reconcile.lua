-- tests/run_reconcile.lua — concurrent-editor detection: out-of-band disk
...
--   nvim --headless -l tests/run_reconcile.lua
...
case("disk change under an unmodified buffer: edit tools error and the buffer reloads", function()
...
case("recovery path: after the error, read then edit succeeds on the new content", function()
  local A = make_session()
...
case("disk change under a MODIFIED buffer: conflict error, both sides kept", function()
  local A = make_session()
  for _, name in ipairs({ "write_file", "edit_file", "patch_file" }) do
    local path, buf = seed(A, "orig\n")
...
case("read_file after a disk change: note + fresh content, no error", function()
  local A = make_session()
...
case("read_file with a modified buffer + disk change: conflict note, buffer content returned", function()
  local A = make_session()
  local path, buf = seed(A, "orig\n")
  vim.api.nvim_buf_set_lines(buf, -1, -1, false, { "UNSAVED" })
  disk_write(path, "EXTERNAL\n")
...
case("file deleted out-of-band: edit errors with the deletion message", function()
  local A = make_session()
  local path, buf = seed(A, "orig\n")
  os.remove(path)
...
case("sibling session's write is an error naming that session and task", function()
...
case("sibling session with NO task: error omits the task clause", function()
...
case("edit_file stamps on success: a third session sees the edit as A's", function()
...
case("fail-open: writing a file this session never read is not checked", function()
...
case("user hand-edit (no stamp): edit stacks on top, no error", function()
  local A = make_session()
...
case("stale stamp (user edited after a sibling's write): edit stacks, no error", function()
  local A, B = make_session(), make_session()
  local path, buf = seed(A, "orig\n")
  tool("write_file", { path = path, content = "from B\n" }, B)
...
case("undo_edit: disk change is an error in all three write modes; history still lists", function()
  local A = make_session()
...
case("sticky conflict: undo_edit's no_reload conflict survives into edit_file, which recovers", function()
...
case("undo_edit: sibling session's write errors with the session-specific message", function()
...
case("undo_edit stamps its write: sibling's next edit errors naming this session", function()
...
case("no external change, single session: tools behave as before", function()
  local A = make_session()
...
case("timestamp-only change (touch): no error, edit proceeds", function()
...
case("file created on disk under a new-file buffer: clear error, no clobber", function()
...
case("minimal ctx (bufnr = 0): tools run without errors, checks fail open", function()
  local ctx0 = { bufnr = 0 }
  local path = vim.fn.tempname() .. ".txt"
