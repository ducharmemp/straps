-- tests/run_quickfix.lua is taken; this is tests/run_run_quickfix.lua —
-- tool.run_quickfix parses command output into the quickfix list via
-- errorformat.
--   nvim --headless -l tests/run_run_quickfix.lua
...
case("parses file:line:col output into the quickfix list, opens it", function()
...
case("clean run empties the list (0 diagnostics)", function()
...
case("non-empty output that matches no efm hints at errorformat", function()
  vim.fn.setqflist({}, "f")
  local out = run({
...
case("captures diagnostics on stderr too", function()
  vim.fn.setqflist({}, "f")
  local out = run({
...
case("open=false does not force the quickfix window (still populates)", function()
  vim.fn.setqflist({}, "f")
  local out = run({
    command = ("printf '%%s\\n' '%s:1:1: x'"):format(fa),
    errorformat = EFM,
...
case("empty command errors clearly", function()
...
case("caps the summary listing but keeps the full list", function()
  vim.fn.setqflist({}, "f")
