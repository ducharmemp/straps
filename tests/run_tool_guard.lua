-- tests/run_tool_guard.lua — the hook.guard.* chain and the tool pipeline.
--   nvim --headless -l tests/run_tool_guard.lua
...
case("pre-freeze: guards define, remove and re-define; scoped define is refused", function()
...
case("hook.on_define never sees a guard define", function()
  _G.on_define_names = {}
...
case("before/block: tool not run, confirm not called, distinct block text", function()
...
case("before/pass (true) and abstain (nil): confirm runs once, tool runs", function()
...
case("mixed: a passes, b blocks → blocked; both guards ran, in seq order", function()
...
case("a raising guard fails closed", function()
...
case("an async before-guard (ctx.await) still blocks", function()
...
case("after: replace, block, order after hook.after_tool, truncated input", function()
...
case("parallel path: gate-all-then-run-all, async before-guard blocks one, async after-guard replaces", function()
...
case("parallel path: cancel during an awaiting gate stubs every block; no job runs", function()
...
case("parallel path: a job's late resolve after loop.stop does not resume it", function()
...
case("subagent-style scope shadow: guards still block; a pass still runs the child's confirm", function()
...
case("define op: blocked from registry.define, tool.registry_define, and a trusted project file", function()
...
case("remove op: a guard keeps hook.confirm in place", function()
...
case("info.executing names the tool whose body is running; nil from a scheduled callback", function()
...
case("info.executing is per-coroutine: overlapping parallel jobs see their own name", function()
...
case("reentrancy: a guard calling define blocks the outer op; composition is not reentrancy", function()
...
case("guard log events: one per guard per op, with verdicts", function()
  reset(); _G.G.b_block = true
  run({ { name = "ping" } })
...
case("frozen after the first run: define/remove refused; loaders and registry_define surface the message", function()
...
case("inert guards: a pass-through chain changes nothing (confirm once, tool runs, four pass verdicts)", function()
...
case("gate edges: run() twice raises; unknown tool is an error result; try_call on a blocked tool raises; guards unreachable via call/bind/call_hooks", function()
  reset()
