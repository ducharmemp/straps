-- tests/run_treesitter.lua — the tree-sitter grammar (tree-sitter-straps/)
--   nvim --headless -l tests/run_treesitter.lua
...
if not (ok_add and added) then
  ...
  if vim.fn.filereadable(so) == 0 then
    ...
    if vim.v.shell_error ~= 0 then
      print("SKIP  run_treesitter: no straps parser on rtp and cc failed: " .. out)
      vim.cmd("quit")
    end
  end
...
case("transcript parses without errors", function()
  assert(not root_node:has_error(), "parse tree has errors: " .. root_node:sexpr())
end)

case("block boundaries agree with state.list_blocks", function()
...
case("highlights and injections queries load", function()
...
case("prose injects markdown, tool bodies inject json, per block", function()
...
case("ftplugin starts the treesitter highlighter for filetype=straps", function()
...
case("apply_file_ref_match underlines refs in windows showing the buffer", function()
...
case("match does not leak into other buffers shown by the window", function()
  local fbuf = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_lines(fbuf, 0, -1, false, LINES)
  vim.bo[fbuf].filetype = "straps"
  vim.cmd("buffer " .. fbuf)
...
case("a split of the session window gets its own match", function()
  local fbuf = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_lines(fbuf, 0, -1, false, LINES)
  vim.bo[fbuf].filetype = "straps"
  vim.cmd("buffer " .. fbuf)
...
case("an in-place filetype change away from straps drops the match", function()
  local fbuf = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_lines(fbuf, 0, -1, false, LINES)
  vim.bo[fbuf].filetype = "straps"
  vim.cmd("buffer " .. fbuf)
