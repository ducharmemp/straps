-- tests/scope_spec.lua — registry scoping + subagents (tool.spawn).
--   busted tests/scope_spec.lua
...
it("session define shadows for the scope chain; global stays untouched", function()
...
it("shadowing reuses the shadowed seq; new scoped entries append", function()
...
it("child scope resolves through its parent's scope", function()
...
it("define_default stays global even while a scope is active", function()
  local buf = vim.api.nvim_create_buf(true, false)
  local prev = registry.set_active_scope(buf)
  local ok, err = pcall(function()
...
it("BufWipeout drops the buffer's scope", function()
  local buf = vim.api.nvim_create_buf(true, false)
  local prev = registry.set_active_scope(buf)
...
it("tool.registry_define defaults to session scope; scope='global' opts out", function()
  local buf = vim.api.nvim_create_buf(true, false)
  local prev = registry.set_active_scope(buf)
  local ok, err = pcall(function()
...
it("tool.registry_define warns on blocking waits in tool sources", function()
  local buf = vim.api.nvim_create_buf(true, false)
  local prev = registry.set_active_scope(buf)
  local ok, err = pcall(function()
...
it("build_tools honors the child tool filter and hides spawn at depth limit", function()
  local buf = vim.api.nvim_create_buf(true, false)
...
it("spawn round trip: child session runs and its answer returns to the parent", function()
  allow_all()
...
it("spawn fires and returns; spawn_wait collects N children run concurrently", function()
  allow_all()
...
it("spawn_wait skips buffers that are not this session's children", function()
  allow_all()
  local parent = vim.api.nvim_create_buf(true, false)
...
it("spawn propagates explicit model/effort to the child buffer", function()
...
it("spawn inherits the parent's per-buffer model/effort when the arg is unset", function()
...
it("readonly child: writes are denied by the child-scope confirm", function()
...
it("fn.readonly_policy is the single source of truth for read-only calls", function()
...
it("spawn depth guard refuses a child spawning a grandchild", function()
  local buf = vim.api.nvim_create_buf(true, false)
  vim.b[buf].straps_spawn_depth = 1
  local out = drive(function(ctx)
    ctx.bufnr = buf
...
it("a crashing child's error comes back to the parent, no hang", function()
  allow_all()
...
it("a hanging child hits the spawn timeout and is stopped", function()
  allow_all()
...
it("cancelling the parent stops a spawned child mid-flight", function()
  allow_all()
...
it("a hand-mangled transcript (deleted tool_result) fails loudly, not forever", function()
  allow_all()
...
it("per-buffer max_turns override bounds a run", function()
  allow_all()
...
it("fn.capability classifies every category and its input-split arms", function()
...
it("spawn allow={edit} grants writes without prompting; ungranted denied", function()
...
it("spawn allow validation rejects bad entries and readonly+allow", function()
  allow_all()
  local parent = vim.api.nvim_create_buf(true, false)
...
it("default hook.confirm honors cap: grants and prompts without them", function()
  local buf = vim.api.nvim_create_buf(true, false)
...
it("ui.auto grants, validates, clears and refuses non-session buffers", function()
