-- tests/spawn_notify_spec.lua — subagent completion notices. Run from the repo root:
--   busted tests/spawn_notify_spec.lua
...
it("hook.on_run_end receives the run's ending reason", function()
  _G.seen_reasons = {}
...
it("a raising subscriber still lets the run clean up, and is logged", function()
...
it("fn.session_notify appends a user block to an idle session, without starting it", function()
...
it("fn.session_notify steers a running session instead of appending", function()
...
it("fn.session_notify's quiet flag controls the steering toast", function()
...
it("fn.session_notify refuses an invalid buffer and empty text", function()
...
it("fn.spawn_notice words each outcome honestly and names the child", function()
  local child = state.new_session()
...
it("a child finishing while the parent runs steers the notice into it", function()
...
it("a child finishing while the parent is idle appends a user block", function()
  provider(ANSWER)
  local parent = state.new_session()
...
it("a child spawn_wait already claimed produces no notice", function()
  provider(ANSWER)
  local parent = state.new_session()
  state.append(parent, "user", nil, "earlier prompt")
...
it("a stopped child is silent even when its run ends with reason error", function()
...
it("an ordinary (non-subagent) run end notifies nobody and logs no error", function()
...
it("config.spawn_notify = false silences the notice", function()
  provider(ANSWER)
...
it("spawn_wait claims its children and drops their queued notices", function()
...
it("the dedupe prefix cannot confuse child 1 with child 12", function()
...
it("spawn_wait survives a child buffer wiped during the wait", function()
...
it("fn.autocmd_bridge still delivers through fn.session_notify", function()
